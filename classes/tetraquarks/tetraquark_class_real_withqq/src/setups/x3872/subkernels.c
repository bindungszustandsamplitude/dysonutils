        ...
        for(size_t iq=0; iq<n_radial; ++iq){
            for(size_t izq=0; izq<n_angular_z; ++izq){
                for(size_t ib3=0; ib3<nb4; ++ib3){
                    for(size_t ib4=0; ib4<nb4; ++ib4){
            kernel[3][SI_K_03(ip,izp, nb1+nb3+ib3 , iq,izq, nb1+nb3+ib4)] = K44[SI_K44(ip,izp,ib3,iq,izq,ib4)];
    }}}}}}
...
for(size_t ip=0; ip<n_radial; ++ip){
    for(size_t izp=0; izp<n_angular_z; ++izp){
        for(size_t iq=0; iq<n_radial; ++iq){
            for(size_t izq=0; izq<n_angular_z; ++izq){
                for(size_t ib1=0; ib1<nb4; ++ib1){
                    for(size_t ib4=0; ib4<nb4; ++ib4){
                        kernel[7][SI_K_07(ip,izp, nb1+ib1 , iq,izq, nb1+ib4)] = K44[SI_K44(ip,izp,ib1,iq,izq,ib4)];
                    }}}}}}
